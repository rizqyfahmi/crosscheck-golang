main() {
    ...
    command docker exec app-database psql -U postgres -c "SELECT 1 FROM pg_database WHERE datname = 'crosscheck'" | grep -q 1 || docker exec app-database psql -U postgres -c "CREATE DATABASE crosscheck"
}
